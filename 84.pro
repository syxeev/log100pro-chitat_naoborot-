...
nondeterm setr(integer, tree, intl)
%nondeterm path(integer, tree, tree)
clauses
run:-
	...
	setr(_,nil,_):-fail. 
	setr(N,tr(N,_,_),[N]). 
	setr(N,tr(M,A,_),[M|L]):-setr(N,A,L). 
	setr(N,tr(M,_,A),[M|L]):-setr(N,A,L).
menu(T):-
	write("Menu"),nl, 
	write("input: "),nl, 
	write(" 1 - to print the tree"),nl,
	write(" 2 - to find path to indicated hight"), nl,
	write(" 0 - exit"),nl, 
	write(">> "), readchar(X),write(X),nl,solve(X,T),nl,nl,menu(T),nl. 
	solve('1', T):-
		write_tree(T).
	solve('2', T):-
		write("Enter the hight-->"),
		readint(I),nl,
		write("path to hight "),write(I),write(" is-->"),
		setr(I, T, L),
		write(L).
